...
def score_place(place, query=""):
    ...
    if reviews:
        score += 10
        try: score += min(int(int(reviews) / 100), 10)
        except: pass
    return score
...
max_workers = st.sidebar.slider("並列スレッド数", min_value=1, max_value=10, value=5)

st.title("📞 店舗電話番号 抽出・上書き補完ツール")
st.markdown("既存の営業リストを読み込み、**電話番号がない・無効な番号の行のみ**をGoogle Mapsから追記補完します。")
...
if uploaded_file is not None:
    try:
        ...
        if st.button("🚀 電話番号の不足分を一括補完する", type="primary", use_container_width=True):
            if not api_key: st.error("❌ SerpAPIキーを設定してください。")
            else:
